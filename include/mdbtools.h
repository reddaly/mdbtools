...
// Note from reddaly@ 4/15/2022: When I added mdbtools back to our codebase, I
// added some statements based on what autoconf generated on my workstation
// build of mdbtools. These statements included enabling iconv using #define
// HAVE_ICONV 1, #define MDBTOOLS_H_HAVE_ICONV_H 1, #define ICONV_CONST const. I
// noticed later that the mdb-tables and mdb-json binaries would not read valid
// database files because column names were not being read properly. Removing
// the ICONV statements seems to have fixed the problem, but I'm not sure why.
...
// Google-specific thread-local storage macro
#ifndef TLS
#define TLS __thread
#endif
...
#define MDB_BIND_SIZE \
  16384  // override with mdb_set_bind_size(MdbHandle*, size_t)
...
typedef enum { MDB_TABLE_SCAN, MDB_LEAF_SCAN, MDB_INDEX_SCAN } MdbStrategy;

typedef enum { MDB_NOFLAGS = 0x00, MDB_WRITABLE = 0x01 } MdbFileFlags;
...
typedef enum {
  ...
  MDB_NOBRACES_4_2_2_2_6, /* "XXXX-XX-XX-XX-XXXXXX" format (matches MS Access
                             ODBC driver) */
} MdbUuidFormat;

#define mdb_is_logical_op(x) (x == MDB_OR || x == MDB_AND || x == MDB_NOT)

#define mdb_is_relational_op(x)                                        \
  (x == MDB_EQUAL || x == MDB_GT || x == MDB_LT || x == MDB_GTEQ ||    \
   x == MDB_LTEQ || x == MDB_NEQ || x == MDB_LIKE || x == MDB_ILIKE || \
   x == MDB_ISNULL || x == MDB_NOTNULL)

enum { MDB_ASC, MDB_DESC };
...
#define MDB_SHEXP_DEFAULT                                           \
  (MDB_SHEXP_CST_NOTNULL | MDB_SHEXP_COMMENTS | MDB_SHEXP_INDEXES | \
   MDB_SHEXP_RELATIONS)
...
MdbCatalogEntry *mdb_get_catalogentry_by_name(MdbHandle *mdb,
                                              const gchar *name);
...
MdbTableDef *mdb_read_table_by_name(MdbHandle *mdb, gchar *table_name,
                                    int obj_type);
...
/* data.c */
int mdb_bind_column_by_name(MdbTableDef *table, gchar *col_name, void *bind_ptr,
                            int *len_ptr);
void mdb_data_dump(MdbTableDef *table);
void mdb_date_to_tm(double td, struct tm *t);
void mdb_tm_to_date(struct tm *t, double *td);
char *mdb_uuid_to_string(
    const void *buf, int start); /* Uses default MDB_BRACES_4_2_2_8 format */
char *mdb_uuid_to_string_fmt(const void *buf, int start, MdbUuidFormat format);
int mdb_bind_column(MdbTableDef *table, int col_num, void *bind_ptr,
                    int *len_ptr);
int mdb_rewind_table(MdbTableDef *table);
int mdb_fetch_row(MdbTableDef *table);
int mdb_is_fixed_col(MdbColumn *col);
char *mdb_col_to_string(MdbHandle *mdb, void *buf, int start, int datatype,
                        int size);
int mdb_find_pg_row(MdbHandle *mdb, int pg_row, void **buf, int *off,
                    size_t *len);
...
size_t mdb_ole_read(MdbHandle *mdb, MdbColumn *col, void *ole_ptr,
                    size_t chunk_size);
...
void mdb_register_backend(
    MdbHandle *mdb, char *backend_name, guint32 capabilities,
    const MdbBackendType *backend_type, const MdbBackendType *type_shortdate,
    const MdbBackendType *type_autonum, const char *short_now,
    const char *long_now, const char *date_fmt, const char *shortdate_fmt,
...
int mdb_print_schema(MdbHandle *mdb, FILE *outfile, char *tabname,
                     char *dbnamespace, guint32 export_options);
void mdb_print_col(FILE *outfile, gchar *col_val, int quote_text, int col_type,
                   int bin_len, char *quote_char, char *escape_char, int flags);
...
int mdb_test_sarg(MdbHandle *mdb, MdbColumn *col, MdbSargNode *node,
                  MdbField *field);
...
int mdb_index_find_next(MdbHandle *mdb, MdbIndex *idx, MdbIndexChain *chain,
...
int mdb_index_find_row(MdbHandle *mdb, MdbIndex *idx, MdbIndexChain *chain,
                       guint32 pg, guint16 row);
...
int mdb_crack_row(MdbTableDef *table, int row_start, size_t row_size,
                  MdbField *fields);
guint16 mdb_add_row_to_pg(MdbTableDef *table, unsigned char *row_buffer,
                          int new_row_size);
int mdb_update_index(MdbTableDef *table, MdbIndex *idx, unsigned int num_fields,
                     MdbField *fields, guint32 pgnum, guint16 rownum);
int mdb_insert_row(MdbTableDef *table, int num_fields, MdbField *fields);
int mdb_pack_row(MdbTableDef *table, unsigned char *row_buffer,
                 unsigned int num_fields, MdbField *fields);
int mdb_replace_row(MdbTableDef *table, int row, void *new_row,
                    int new_row_size);
...
gint32 mdb_map_find_next(MdbHandle *mdb, unsigned char *map,
                         unsigned int map_sz, guint32 start_pg);
...
void mdb_fill_temp_col(MdbColumn *tcol, char *col_name, int col_size,
                       int col_type, int is_fixed);
void mdb_fill_temp_field(MdbField *field, void *value, int siz, int is_fixed,
...
/* iconv.c */
int mdb_unicode2ascii(MdbHandle *mdb, const char *src, size_t slen, char *dest,
                      size_t dlen);
int mdb_ascii2unicode(MdbHandle *mdb, const char *src, size_t slen, char *dest,
                      size_t dlen);
